...
def get_datetime_range(time_unit='hourly'):
    if time_unit == 'hourly':
        ...
        return pd.date_range(start=start, end=today(), freq='D').strftime('%Y-%m-%d')
...
def insert_hourly_cell_data():
    setproctitle('__insert_sa_cell__')
    load_dotenv()
...
if __name__ == '__main__':
    insert_hourly_cell_data()
    schedule.every().hour.at(":08").do(insert_hourly_cell_data)
    while True:
