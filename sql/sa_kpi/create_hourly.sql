insert into dnb.sa_kpi_results_hourly_hourly.dc_e_nr_events_nrcellcu_standard_sa_raw
with dt as (select t1.date_id,
            ...
            where t1.date_id not in (select date_id
                                     from sa_kpi_results_hourly_hourly.dc_e_nr_events_nrcellcu_standard_sa_raw
                                     group by date_id)
...
with dt as (select t1.date_id,
                   ...
                   "Cluster_ID",
                   nr_name,
                   split_part((flex_filtername)::text, '_'::text, 1) as flex_filtername,
                   mean_user_dl_throughput_nr_nom,
                   mean_user_dl_throughput_nr_den,
                   mean_user_ul_throughput_nr_nom,
                   mean_user_ul_throughput_nr_den,
...
with dt as (select dt.date_id,
            ...
            where dt.date_id not in (select date_id
                                     ...
                                     group by date_id)
            and dt.date_id >= current_date - interval '14 days'),
     dt2 as (select date_id,
                    "Region"                                    as region,
                    "Cluster_ID"                                as cluster_id,
                    flex_filtername,
